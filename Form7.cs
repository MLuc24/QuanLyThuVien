    ...
    public partial class Form7 : Form
    {
            ...
            using (SqlConnection connection = new SqlConnection(kn))
            {
                ...
                try
                {
                    ...
                    foreach (ListViewItem item in lvSachMuon.Items)
                    {
                        ...
                        string queryCTMuonTra = "INSERT INTO tblCTmuontra (sMaphieu, sMasach, dNgayhentra, iSlmuon, sGhichu, isDeleted) " +
                                                "VALUES (@MaPhieu, @MaSach, @NgayHenTra, @SoLuong, @GhiChu, 0)";
        ...
        private DataTable SachChuaMuon(string maPhieu)
        {
            ...
            AND s.sMasach NOT IN (
                ...
                WHERE ct.sMaphieu = @MaPhieu AND ct.isDeleted = 0
            )";
                SqlCommand cmd = new SqlCommand(query, connection);
                cmd.Parameters.AddWithValue("@MaPhieu", maPhieu);
        ...
        private void LoadDataTra()
        {
            using (SqlConnection connection = new SqlConnection(kn))
            {
                ...
                pm = new DataTable();
                adapter.Fill(pm);
                dgvPhieuMuonTra.DataSource = pm;
                dgvPhieuMuonTra.Columns["Ngày mượn"].DefaultCellStyle.Format = "dd/MM/yyyy";
        ...
        private void DisplayThongTinPhieuMuon()
        {
            ...
            int soLuongMuon = Convert.ToInt32(selectedRow.Cells["SL"].Value);

            // Lấy tên độc giả từ bảng tblDocGia
            string tenDocGia = GetTenDocGia(maDocGia);

            // Lấy tên sách từ bảng tblSach
            ...
            txtSLM.Text = soLuongMuon.ToString();

            dtpNT.Value = ngayMuon.AddDays(7);
        }
        ...
        // Phương thức lấy tên sách từ mã sách
        private string GetTenSach(string maSach)
        {
            using (SqlConnection connection = new SqlConnection(kn))
            {
                connection.Open();
                string query = "SELECT sTensach FROM tblSach WHERE sMasach = @MaSach";
                SqlCommand cmd = new SqlCommand(query, connection);
                cmd.Parameters.AddWithValue("@MaSach", maSach);
                object result = cmd.ExecuteScalar();
                return result != null ? result.ToString() : "Không tìm thấy";
        ...
        private void EnableControlsForTraSach()
        {
            ...
            dtpNT.Enabled = true;
            txtGC.Enabled = true;
            btnTra.Enabled = true; // Kích hoạt nút trả sách
        }
        ...
        private void InitializeControlsPT()
        {
            ...
            txtSLM.Clear();
            cboTinhTrangTra.Items.Clear(); // Thêm dòng này để xóa các mục cũ
            cboTinhTrangTra.Items.AddRange(new string[] { "Hỏng", "Nguyên vẹn", "Hỏng 1/2" }); // Thêm các mục mới
            cboTinhTrangTra.SelectedIndex = -1;
            dtpNT.Value = DateTime.Now.AddDays(7); // Ngày trả mặc định là ngày hiện tại
            txtGC.Clear();
        ...
        {
            // Kiểm tra xem đã chọn tình trạng trả chưa
            if (cboTinhTrangTra.SelectedIndex == -1)
            {
                MessageBox.Show("Vui lòng chọn tình trạng trả!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // Xác nhận với người dùng
            DialogResult result = MessageBox.Show("Bạn có chắc chắn muốn trả sách không?", "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void TraSach()
        {
            using (SqlConnection connection = new SqlConnection(kn))
            {
                ...
                try
                {
                    ...
                    string maSach = GetMaSachFromTenSach(txtMS.Text);

                    // Kiểm tra xem mã sách có hợp lệ không
                    if (string.IsNullOrEmpty(maSach))
                    {
                        MessageBox.Show("Không tìm thấy mã sách tương ứng!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }

                    // Cập nhật isDeleted = 1 trong tblCTmuontra
                    string queryUpdate = @"
                UPDATE tblCTmuontra
                SET isDeleted = 1
                WHERE sMaphieu = @MaPhieu AND sMasach = @MaSach";

                    SqlCommand cmdUpdate = new SqlCommand(queryUpdate, connection, transaction);
                    cmdUpdate.Parameters.AddWithValue("@MaPhieu", txtMaPhieuTra.Text);
                    cmdUpdate.Parameters.AddWithValue("@MaSach", maSach);
                    cmdUpdate.ExecuteNonQuery();

                    // Chèn dữ liệu vào tblPileutra
                    string queryInsert = @"
                INSERT INTO tblPhieutra (sMaphieu, sMasach,iSoluongmuon, dNgayhentra, dNgaytra, sTinhtrang, sGhichu)
                VALUES (@MaPhieu, @MaSach,@Sl, @NgayHenTra, @NgayTra, @TinhTrang, @GhiChu)";

                    SqlCommand cmdInsert = new SqlCommand(queryInsert, connection, transaction);
                    cmdInsert.Parameters.AddWithValue("@MaPhieu", txtMaPhieuTra.Text);
                    cmdInsert.Parameters.AddWithValue("@MaSach", maSach);
                    cmdInsert.Parameters.AddWithValue("@Sl", Convert.ToInt32(txtSLM.Text));
                    cmdInsert.Parameters.AddWithValue("@NgayHenTra", dtpNHT.Value);
                    cmdInsert.Parameters.AddWithValue("@NgayTra", dtpNT.Value);
                    cmdInsert.Parameters.AddWithValue("@TinhTrang", cboTinhTrangTra.SelectedItem.ToString());
        ...
        private string GetMaSachFromTenSach(string tenSach)
        {
            ...
            {
                connection.Open();
                string query = "SELECT sMasach FROM tblSach WHERE sTensach = @TenSach";
                SqlCommand cmd = new SqlCommand(query, connection);
        ...
        private void dtpNT_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (dtpNT.Value <= dtpNM.Value)
            {
                MessageBox.Show("Ngày trả không được nhỏ hơn ngày mượn!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                dtpNT.Value = dtpNM.Value.AddDays(7); // Đặt lại ngày trả bằng ngày mượn
            }
        }
        ...
        private void PerformSearchTra()
        {
            ...
            {
                query = "SELECT * FROM v_DSPMT WHERE [Mã sách] = @Keyword";
            }
            ...
            using (SqlConnection cnn = new SqlConnection(kn))
            {
                using (SqlCommand cmd = new SqlCommand(query, cnn))
                {
                    ...
                    using (SqlDataAdapter ad = new SqlDataAdapter(cmd))
                    {
                        DataTable searchResult = new DataTable();
                        ad.Fill(searchResult);
                        dgvPhieuMuonTra.DataSource = searchResult;
                        dgvPhieuMuonTra.Refresh(); // Cập nhật giao diện ngay lập tức
                    }
                }
            }
        }
